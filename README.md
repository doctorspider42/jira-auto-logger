...
- **Weekday auto logger** — run at a configured local time using a 24-hour schedule and the most recent project selection. Confirmation mode shows a system notification and opens generated entries for review; full-auto mode generates and submits the remaining day directly. Existing Tempo hours are taken into account, one scheduled attempt is made per day, and a **Run now** action is available for immediate/manual runs.
