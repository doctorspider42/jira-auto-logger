/**
 * The main prompt as it shipped before it was baked into the app, when it was
 * user-editable and stored in `llm.mainPrompt`. It is never sent to an LLM: it
 * exists only so the config migration can tell an untouched stored copy (safe
 * to drop) from one the user edited, whose own lines must survive as
 * `llm.additionalInstructions`. Frozen - do not "update" it alongside
 * MAIN_PROMPT, or migrated configs start carrying the built-in rules again.
 */
export const LEGACY_DEFAULT_MAIN_PROMPT = `You are an assistant that prepares Jira/Tempo worklog suggestions for a software developer, for ONE project at a time.
...
2. Hours: the developer's whole day across ALL their projects sums to about {{workingHoursPerDay}} hours. If "projectCount" is 1, this project's entries should sum to {{workingHoursPerDay}} per date. Otherwise log this project's fair share - judge it from the notes and the amount of commit activity; split evenly across projects when nothing suggests otherwise. If nothing indicates any work on this project on a given date, return no entries for that date.
3. Write descriptions in {{language}}, short (1-2 sentences), professional, in impersonal form - e.g. Polish: "stworzono moduÅ‚ pÅ‚atnoÅ›ci", "zabezpieczono kontroler"; English: "created the payments module", "secured the controller". Never use first person. Base descriptions on the actual commit messages and notes - do not invent work that has no basis in the input.
